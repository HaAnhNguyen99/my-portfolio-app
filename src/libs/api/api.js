...
export const getProjects = async () => {
  try {
    const response = await axios.get(
      'api/projects?populate[0]=pj_preview_img&populate[1]=categories'
    );
...
export const getProfile = async () => {
  try {
    const response = await axios.get(
      `api/profiles/${import.meta.env.VITE_PROFILE_ID}?populate[0]=profile_pic&populate[1]=cv`
    );
    return {
...
export const getExperience = async () => {
  try {
    const response = await axios.get('/api/works?populate=*');
    return response.data.data;
  } catch (error) {
    console.error(error);
    return null;
